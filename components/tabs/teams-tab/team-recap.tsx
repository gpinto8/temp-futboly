...
export const TeamRecap = ({team, index, selectedTeamIndex, setSelectedTeamIndex}) => {
    return (
        <CustomCard id={'team-card-' + index} key={index} style="gray" className={selectedTeamIndex === index ? 'row-span-2' : ''} >
            <div
                className="flex flex-col justify-center items-start gap-2"
                onClick={() => selectedTeamIndex === index ? setSelectedTeamIndex(null) : setSelectedTeamIndex(index) }>
                ...
                <div className="flex flex-row gap-8 justify-start items-center my-2">
                    ...
                    <div className="flex flex-col gap-2 justify-start items-center">
                        ...
                        <div className="flex justify-center items-center gap-2">
                            ...
                            <p className="font-semibold text-gray-900">{team.leaguePosition}</p>
                        </div>
                    </div>
                </div>
                ...
                <button className="mt-4 px-4 py-2 bg-gray-200 rounded self-center shadow-md hover:scale-105 flex justify-center items-center gap-2">
                    ...
                    <CustomImage imageKey={selectedTeamIndex === index ? 'EXPAND' : 'CLOSE_ICON'} className="h-4 w-4 ml-2" />
                </button>
                            ...
                            {team.players.map((player, index) => (
                                <div key={index} className="flex justify-around items-center gap-4">
                                    ...
                                    <div className="flex justify-around items-center gap-2">
                                        <CustomImage forceSrc={player.image} className="h-8 w-8 rounded-full border border-black" />
