...
export const NoMatches = () => {
    ...
    const currentCompetition = useAppSelector((state) => state.competition.activeCompetition);
    const { scheduleCompetitionMatches } = useSetCompetitions();

    const textForNoMatches = leagueAdmin === user.id ?
                ...
                <div className="container flex flex-col items-center px-4 py-12 mx-auto text-center">
                    ...
                    { leagueAdmin === user.id && (
                        <div className="inline-flex w-full mt-6 sm:w-auto">
                            <CustomButton label="Generate Match Schedule" handleClick={generateMatchSchedule} />
                        </div>
                    )} 
                </div>
            </section>
