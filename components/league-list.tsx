...
import { ColumnsProps, RowsProps, CustomTable } from '@/components/custom/custom-table';
...
const columns: ColumnsProps<LeaguesColumnKeysProps> = [
  { id: 'PRIVATE', label: ' ', minWidth: 25, align: 'center' },
  { id: 'NAME', label: 'Name', minWidth: 50, align: 'center' },
  { id: 'WEEK', label: 'Week', minWidth: 50, align: 'center' },
...
const getRows = (leagues: Array<LeaguesCollectionProps>) => {
    return leagues.map((league: LeaguesCollectionProps) => {
        ...
        return {
            PRIVATE: <CustomImage imageKey={league.isPrivate ? "PRIVATE" : "PUBLIC"} width={20} height={20} />,
            NAME: league.name,
            WEEK: 'Week 1',
            PLAYERS: players + " / 10",
            ACTIONS: <JoinPublicLeagueModal league={league as LeaguesCollectionProps}/>,
        }
    });
}
...
export const LeagueList = () => {
    ...
    const [rows, setRows] = useState<RowsProps<LeaguesColumnKeysProps> | never[]>();

    useEffect(() => {
        const fetchData = async () => {
            if (!hasMore) return;
            const data = await getAllLeaguesByChunks(itemCounter === 0 ? undefined : lastVisible, 5);
            if (!data) return;
            if (rows && rows?.length > 0 && Array.isArray(rows)) {
                setRows([...rows, ...getRows(data.leagues as LeaguesCollectionProps[])]);
            } else {
        ...
        <div>
            ...
            { (rows) && (rows.length > 0) ? (
                <div className="mt-4 rounded-xl">
                    ...
                    onEndReached={() => {hasMore ? setItemCounter(itemCounter + 1) : null /*We could handle better for example showing "no more leagues"*/}}
                    isComplete={{value: !hasMore, text: "No more leagues"}}
                    />
                </div>
            ) : (
                <div className="flex flex-col items-center justify-center gap-4 mt-4">
                    <p className="text-lg font-semibold text-gray-500">No leagues found</p>
                </div>
            )}
        </div>
    )
