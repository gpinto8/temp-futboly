...
export const CustomField = ({id, players, module, className}: CustomFieldProps) => {
    ...
    useEffect(() => {
        const copyPlayers = [...players].map((player, index) => ({...player, pos: index}));
    ...
    return (
        <div id={id} className={"w-full h-full grid grid-rows-4 " + className} style={{
            backgroundImage: "url('/assets/gray-field.svg')",
            backgroundRepeat: "no-repeat",
            backgroundPosition: "center",
            backgroundSize: "100% 100%",
        }}>
            <div id="formation-fw" className="row-span-1 flex w-full h-full justify-around items-center">
                {forward.map((player: any, index) => (
                    <PlayerSection key={index} player={player}/>
                ))}
            </div>
            <div id="formation-mf" className="row-span-1 flex w-full h-full justify-around items-center">
                {midfield.map((player: any, index) => (
                    <PlayerSection key={index} player={player}/>
                ))}
            </div>
            <div id="formation-def" className="row-span-1 flex w-full h-full justify-around items-center">
                {defense.map((player: any, index) => (
                    <PlayerSection key={index} player={player}/>
                ))}
            </div>
            <div id="formation-gk" className="row-span-1 flex w-full h-full justify-around items-center">
...
const PlayerSection = ({player}: {player: PlayerType | null}) => {
    return player ? (
        <div className="border border-black p-1 rounded-full bg-white flex flex-col justify-center items-center size-24">
            <CustomImage forceSrc={player.image} className={"size-10 border rounded-full" + (player.isCaptain ? " border-error-500" : "")}/>
