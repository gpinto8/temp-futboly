...
export const CustomField = ({
  ...
  return player_module.length === 1 && teams === 1 ? (
    ...
    >
      <TeamSection
        players={player_module[0].players}
        module={player_module[0].module}
      />
    </div>
  ) : (
    <div
    ...
    >
      ...
      />
      <TeamSection
        players={player_module[1].players}
        module={player_module[1].module}
      />
    </div>
  );
...
const TeamSection = ({
  ...
  return (
    <div className={'w-full h-full grid grid-rows-4 '}>
      {!isReversed
        ? teamComposition.map((jsxElement) => jsxElement)
        : teamComposition.toReversed().map((jsxElement) => jsxElement)}
    </div>
  );
};
