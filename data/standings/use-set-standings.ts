...
import { MatchScheduleProps, StandingsResults, ShortTeamPropsStandings, ShortTeamProps } from '@/firebase/db-types';
...
export const useSetStandings = () => {
    ...
    const calculateAndSaveStandings = async (competitionId: string) => {
        ...
        const completeStandings: ShortTeamPropsStandings[] = mapTeamMatches
            ...
            .toReversed()
            .map((teamStandingsResult, index) => { return {...teamStandingsResult, position: index + 1}});
        const updateStandingResult = await firestoreMethods("competitions", competitionData.id as any).replaceField("standings", completeStandings);
...
function calculateMatchResult(match: MatchScheduleProps, team: ShortTeamProps): "W" | "L" | "D" | null {
